...
def aggregate_mean(X, adj, D=None, self_loop=False, device='cpu'):
    ...
    if D is not None:
        D = D.unsqueeze(-1)
    if not len(X.shape) == len(adj.shape):
        adj = adj.unsqueeze(-1)
    # D = torch.sum(adj, -1, keepdim=True)
    X_sum = torch.sum(torch.mul(X, adj), dim=2)
    # X_sum = torch.sum(torch.mul(X, adj.unsqueeze(-1)), dim=2)  # todo: dont need unsqueezing
    X_mean = torch.div(X_sum, D)  # B N D  /  B N
    return X_mean


def aggregate_max(X, adj, D=None, min_value=-math.inf, self_loop=False, device='cpu'):
    (B, N, N, Din) = X.shape
    device = X.device
...
def aggregate_min(X, adj, D=None, max_value=math.inf, self_loop=False, device='cpu'):
    (B, N, N, Din) = X.shape
    device = X.device
...
def aggregate_std(X, adj, D=None, self_loop=False, device='cpu'):
    ...
    std = torch.sqrt(aggregate_var(X, adj, D, self_loop, device) + EPS)  # sqrt(mean_squares_X - mean_X^2)
    return std


def aggregate_var(X, adj, D, self_loop=False, device='cpu'):
    ...
    X_sum_squares = torch.sum(torch.mul(torch.mul(X, X), adj), dim=2)
    X_mean_squares = torch.div(X_sum_squares, D.unsqueeze(-1))  # D^{-1} A X^2
    X_mean = aggregate_mean(X, adj, D)  # D^{-1} A X
