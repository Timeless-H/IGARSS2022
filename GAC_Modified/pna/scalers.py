...
def scale_amplification(X, adj, avg_d=None):
    ...
    scale = (torch.log(adj + 1) / avg_d).unsqueeze(-1)  # adj here is D for all intents & purpose
...
def scale_attenuation(X, adj, avg_d=None):
    ...
    scale = (avg_d / torch.log(adj + 1)).unsqueeze(-1)  # adj here is D for all intents & purpose
    X_scaled = torch.mul(scale, X)
    return X_scaled
